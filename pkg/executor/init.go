...
import (
	...
	"code.cloudfoundry.org/k8s-garden-client/pkg/k8sgarden/log"
...
// As soon as we get rid of this Initializer code copy,
// we can revert the type of config parameter back to ExecutorConfig.
func Initialize(
	logger lager.Logger,
	config config.RepConfig,
	cellID string,
	zone string,
	...
	*containermetrics.StatsReporter,
	...
	if !mgr.GetCache().WaitForCacheSync(context.Background()) {
		logger.Error("failed-to-wait-for-cache-sync", err)
		os.Exit(1)
	}

	kubeletClient, err := newKubeletClientFromConfig(mgr.GetConfig(), os.Getenv("NODE_IP"), "10250")
	if err != nil {
		return nil, nil, grouper.Members{}, err
	}
	...
	gardenClientFactory := k8sgarden.NewFactory(gardenClient)
	// END GARDEN.CLIENT INSTANTIATION FOR KUBERNETES
	...
	assetTLSConfig, err := TLSConfigFromConfig(logger, certsRetriever, config.ExecutorConfig)
	if err != nil {
		return nil, nil, grouper.Members{}, err
	}
	...
	totalCapacity, err := fetchCapacity(logger, gardenClient, config.ExecutorConfig)
	if err != nil {
		return nil, nil, grouper.Members{}, err
	}
	...
	credManager, err := CredManagerFromConfig(logger, metronClient, config.ExecutorConfig, clock, instanceIdentityHandler, proxyConfigHandler)
	if err != nil {
		return nil, nil, grouper.Members{}, err
	}
	...
	return depotClient, containerStatsReporter,
		grouper.Members{
			...
			{Name: "metrics-reporter", Runner: &metrics.Reporter{
				...
				Tags:           map[string]string{"zone": config.Zone},
			}},
...
func fetchCapacity(logger lager.Logger, gardenClient GardenClient.Client, config initializer.ExecutorConfig) (executor.ExecutorResources, error) {
...
func TLSConfigFromConfig(logger lager.Logger, certsRetriever CertPoolRetriever, config initializer.ExecutorConfig) (*tls.Config, error) {
...
func CredManagerFromConfig(logger lager.Logger, metronClient loggingclient.IngressClient, config initializer.ExecutorConfig, clock clock.Clock, handlers ...containerstore.CredentialHandler) (containerstore.CredManager, error) {
